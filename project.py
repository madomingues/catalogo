...
def showBeers(estilo_id):
  ...
  if 'username' not in login_session:
    ...
    return render_template('showBeers.html',cervejas=cervejas,estilo=estilo,
                            estilo_id = estilo_id)
...
def newBeer(estilo_id):
  if 'username' not in login_session:
    ...
    if request.method =='POST':
      ...
      return render_template('newBeer.html',estilo_id=estilo_id,estilo=estilo)
  ...
  if 'username' not in login_session:
    ...
    if request.method=='POST':
      ...
      if request.form['temperatura']:
        editedBeer.temperatua=request.form['temperatura']
      if request.form['cor']:
      ...
      return render_template('editBeer.html',cerveja=editedBeer,estilo=estilo,
                              estilo_id=estilo_id)

@app.route('/1000cervejas/<int:estilo_id>/beers/<int:cerveja_id>/delete', 
            methods=['GET','POST'])
def deleteBeer(estilo_id,cerveja_id):
  if 'usename' not in login_session:
    return render_template('login.html')
  else:
    estilo =session.query(BeerStyle).filter_by(id=estilo_id).one()
...
def descrBeer(estilo_id,cerveja_id):
  ...
  if 'username' not in login_session:
    ...
    return render_template('descrBeer.html',estilo=estilo,cerveja=beerDescr)
